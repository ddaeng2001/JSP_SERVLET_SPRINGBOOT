...
<%!
	...
	public static void disConn(Connection conn) throws Exception //연결해제 : 계속 연결 중이면 과부화가 발생할 수도 있으니!
	{
		conn.close();
	}
...
<%
	...
	if(url.contains("/join")){
	 	//url에서 join을 포함하고 있다면(회원가입 요청을 한다면)
	 	...
	 	if(result>0) //result>0 : 정상적인 회원가입이 됐다는 말이기에 login.jsp페이지를 만들어서 넘겨주면 됨
	 		response.sendRedirect("./login.jsp"); //login 폼으로 이동
	 	else
	 		;
